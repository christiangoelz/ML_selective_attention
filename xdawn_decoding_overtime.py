...
def run_classification(data, param):
    subj = data[0]
    print(subj)
    if data[1] == None:
        return (subj, None)
    epochs = data[1]
    time_res_scores = task_classification(
        epochs, param, subj, time_resovled=True, permutation=False)
...
def permuation_score(epochs, param, subj, n_iterations):
    ...
    for i in range(n_iterations):
        this_scores = task_classification(
            epochs, param, subj, time_resovled=False, permutation=True, perm_seed=i)
